...
library("plyr")
library("reshape2")
data.all <- read.table('OTUtable_tempFWDonly.txt')
mytable = otu_table(data.all, taxa_are_rows=TRUE,errorIfNULL=TRUE)
env.all <- read.table('NBP1910_envdataCTD_2021c.txt')
envdata = sample_data(data.frame(cbind(data.frame(scale(env.all[c(10:30)])),env.all[c(1:9)])))
...
rarefaction_curve_data <- calculate_rarefaction_curves(psdata, c('Observed', 'Chao1'), rep(c(1, 10, 100, 1000, 1:100 * 10000), each = 10))
