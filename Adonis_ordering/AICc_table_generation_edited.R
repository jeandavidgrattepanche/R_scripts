...
AICc.table.2var <- function(sig.vars, control.var.char = NULL, c.var = 0, matrix.char, perm, type = "AICc", method = "bray", data) {
...
#	combo.list <- combinat::permn(sig.vars) # not option for lenght
	combo.list <- DescTools::CombSet(sig.vars, 2, ord= TRUE, as.list = TRUE)
...
AICc.table.Nvar <- function(sig.vars, control.var.char = NULL, c.var = 0, matrix.char, perm, n.var = 1, composite = FALSE, type = "AICc", method = "bray", df) {
...
#	combo.list <- combn(x = sig.vars, m = n.var, simplify = FALSE)
	combo.list <- DescTools::CombSet(sig.vars, n.var, ord= TRUE, as.list = TRUE)

	varcomb.N.AICc <- tibble(variables = rep("var.name", length(combo.list)),
							 AICc.values = rep(0),
							 `Pseudo-_F_` = rep(0),
							 `p-value` = rep(0),
							 `Var Explnd` = rep(0),
...
#	for (r in 1:choose(length(sig.vars), n.var)) {
	for (r in 1:length(combo.list)) {
		# label the row with variable names
		varcomb.N.AICc[r,1] <- paste(control.var.char, paste(combo.list[[r]], collapse = " + "))

		# create a temporary PERMANOVA to take info from
		temp <- adonis2(
			as.formula(paste0(
				deparse(quote(matrix.char)),
				" ~ ",
				control.var.char,
				paste0(combo.list[[r]], collapse =  "+")
			)),
...
#		varcomb.N.AICc$`Var Explnd`[r] <- temp$SumOfSqs[1] / temp$SumOfSqs[3]
		varcomb.N.AICc$`Var Explnd`[r] <- 1-(temp$SumOfSqs[1] / temp$SumOfSqs[length(temp$SumOfSqs)])
...
if (composite == FALSE) {
	...
	varcomb.N.AICc$`Relative Likelihood` <-
		exp(-.5 * (varcomb.N.AICc$AICc.values - min(varcomb.N.AICc$AICc.values)))
	}
...
AICc.table.all <- function(sig.vars, control.var.char = NULL, matrix.char, perm = 999, comb.incl = 1, extra.var = FALSE, extra.var.char = NULL, type = "AICc", method = "bray", df) {
	...
	if (!is.null(control.var.char)) {
		...
		temp <- AICc.table.Nvar(sig.vars = control.var.char, control.var.char = NULL,
								...
								type = type, method = method, perm = perm, df = df)
