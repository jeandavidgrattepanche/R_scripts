...
write.table(AICresults.pico, file="AIC_Table_pico_BD2d.txt", append=F, sep="\t")

torun <- subset_samples(physeqb, size == "nano")
env.var.pico <- c( "latitude", "water_temp", "conductivity" , "Bprod", "ice", "depth", "ZML_TS", "oxygen", "beam_trans", "fluorescence", "NO2NO3", "NH4", "PO4")
AICresults.pico <- AICc.table.all(env.var.pico, matrix.char=distance(torun,"jaccard"),perm=999,method="jaccard", df=as(sample_data(torun),"data.frame"),comb.incl=c(3,4,5,6,7))
write.table(AICresults.pico, file="AIC_Table_nano_BD2d.txt", append=F, sep="\t")

torun <- subset_samples(physeqb, size == "micro")
env.var.pico <- c( "latitude", "water_temp", "conductivity" , "Bprod", "ice", "depth", "ZML_TS", "oxygen", "beam_trans", "fluorescence", "NO2NO3", "NH4", "PO4")
AICresults.pico <- AICc.table.all(env.var.pico, matrix.char=distance(torun,"jaccard"),perm=999,method="jaccard", df=as(sample_data(torun),"data.frame"),comb.incl=c(3,4,5,6,7))
write.table(AICresults.pico, file="AIC_Table_micro_BD2d.txt", append=F, sep="\t")
